...
ptest(_Config) ->
    ...
    otters:span_pstart("test_span"),
    otters:span_plog("started"),
    otters:span_plog("α =:= ω"),
    otters:span_ptag("result", "ok"),
    otters:span_pend(),
    timer:sleep(200),
...
ftest(_Config) ->
    ...
    S1 = otters:span_start("test_span"),
    S2 = otters:span_log(S1, "started"),
    S3 = otters:span_tag(S2, "result", "ok"),
    S4 = otters:span_log(S3, "α =:= ω"),
    S5 = otters:span_log(S4, 123456),
    S6 = otters:span_log(S5, 'this is a atom'),
    S7 = otters:span_log(S6, io_lib:format("int: ~w, float: ~f, hex: ~.16B, Span: ~p",
					  [1, 1.0, 1, S6])),
    S8 = otters:span_log(S7, S7),
    S9 = otters:span_log(S8, fun() -> "result of function" end),
    otters:span_end(S9),
    timer:sleep(200),
    ?assert(meck:validate(ibrowse)),
    meck:unload(ibrowse).
